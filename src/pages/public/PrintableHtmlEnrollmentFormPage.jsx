...
import PrintableHtmlEnrollmentForm from '../../enrollment/htmlForm/PrintableHtmlEnrollmentForm.jsx';
...
export default function PrintableHtmlEnrollmentFormPage() {
  const { portalName, school, branding, activeSchoolId } = usePortalConfig();
  const { user } = useAuth();
  ...
  const logoUrl = branding?.logoUrl || branding?.logoIconUrl || null;
  ...
  if (loadState.loading || !loadState.initialData) {
    return (
      <div className="printable-html-enrollment-loading">
        <NetworkBanner />
  ...
  return (
    <div className="printable-html-enrollment-page">
      <NetworkBanner />
      <div className="no-print printable-html-enrollment-back">
        <Link to={tenantPath('/enroll')} className="enrollment-back-link inline-flex items-center gap-1">
          <ArrowLeft size={14} />
          Back to Enrollment
        </Link>
      </div>
      <PrintableHtmlEnrollmentForm
        key={loadState.resolvedAppId || 'new-html-form'}
